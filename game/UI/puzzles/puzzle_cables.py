...
El jugador debe conectar pares de puertos del MISMO PROTOCOLO
(TCP con TCP, HTTP con HTTP, etc.). El color es decorativo,
la verificación se hace por nombre de protocolo.
"""
...
# Pool completo de protocolos disponibles
PROTOCOLOS = [
...
class PuzzleCables(BasePuzzle):
    TITULO = "PATCH PANEL"
    HINT = "Click en 2 puertos del mismo protocolo para conectar  |  ESC para salir"
    DURACION_ERROR = 320
    RADIO_HIT = 28

    def _construir(self):
        # Cantidad de cables según dificultad
        ...
        # Construir 8-10 puertos: 2 por cable
        self.puertos = []
        ...
        # Posicionar puertos: columna izq y columna der
        # Mezclar orden vertical de la columna derecha para que no estén
        # alienados.
        ...
        # Asignar puertos a cables
        for cab in self.cables:
            cab["puerto_izq"] = [p["id"] for p in self.puertos if p["lado"] == "L"][cab["id"]]
            cab["puerto_der"] = [p["id"] for p in self.puertos if p["lado"] == "R" and p["cable_id"] == cab["id"]][0]
    ...
    def _dibujar_subclase(self):
        ...
        for cab in self.cables:
            ...
            pygame.draw.line(
                self.pantalla, (max(0, color[0] - 60), max(0, color[1] - 60), max(0, color[2] - 60)),
                (a["x"], a["y"]), (b["x"], b["y"]), 8
            )
            pygame.draw.line(self.pantalla, color, (a["x"], a["y"]), (b["x"], b["y"]), 4)
            pygame.draw.line(
                self.pantalla, (255, 255, 255),
                (a["x"], a["y"]), (b["x"], b["y"]), 1
            )

        # Cable temporal: puerto seleccionado → cursor
        if self.puerto_seleccionado is not None:
            ...
            self.pantalla.blit(halo, (0, 0))
            pygame.draw.line(
                self.pantalla, (max(0, color[0] - 40), max(0, color[1] - 40), max(0, color[2] - 40)),
                (sp["x"], sp["y"]), (mx, my), 6
            )
            pygame.draw.line(self.pantalla, color, (sp["x"], sp["y"]), (mx, my), 3)
            pygame.draw.line(
                self.pantalla, (220, 230, 240),
                (sp["x"], sp["y"]), (mx, my), 1
            )
    ...
    def _dibujar_puerto(self, p):
        ...
        if en_error:
            ...
            surf = pygame.Surface((radio_efectivo * 2 + 6, radio_efectivo * 2 + 6), pygame.SRCALPHA)
            pygame.draw.circle(
                surf, (255, 60, 60),
                (radio_efectivo + 3, radio_efectivo + 3), radio_efectivo
            )
            pygame.draw.circle(
                surf, (255, 255, 255),
                (radio_efectivo + 3, radio_efectivo + 3), radio_efectivo, 3
            )
            self.pantalla.blit(surf, (x - radio_efectivo - 3, y - radio_efectivo - 3))
            return
        ...
        if self.puerto_seleccionado == p["id"]:
            ...
            pygame.draw.circle(
                halo, (*color, 120 + int(pulso * 80)),
                (radio * 2.5, radio * 2.5), radio + 8 + int(pulso * 4)
            )
            self.pantalla.blit(halo, (x - radio * 2.5, y - radio * 2.5))
        ...
        pygame.draw.circle(
            surf, (255, 255, 255),
            (radio + 2, radio + 2), radio, 3
        )

        # Si está conectado, mostrar tick
        if p["conectado_a"] is not None:
            pygame.draw.line(
                surf, (255, 255, 255),
                (radio - 6, radio + 2), (radio - 2, radio + 6), 3
            )
            pygame.draw.line(
                surf, (255, 255, 255),
                (radio - 2, radio + 6), (radio + 8, radio - 6), 3
            )

        self.pantalla.blit(surf, (x - radio - 2, y - radio - 2))

        # Etiqueta del protocolo (centrada debajo)
        etiqueta = self.fuente_etiqueta.render(p["protocolo"], True, (220, 230, 240))
        etiqueta_x = x - etiqueta.get_width() // 2
        etiqueta_y = y + radio + 10
        # Fondo para legibilidad
        pad_x, pad_y = 8, 4
        fondo = pygame.Surface(
            (etiqueta.get_width() + pad_x * 2, etiqueta.get_height() + pad_y * 2),
            pygame.SRCALPHA
        )
        ...
        self.pantalla.blit(fondo, (etiqueta_x - pad_x, etiqueta_y - pad_y))
